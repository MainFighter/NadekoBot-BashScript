...
if [ $choice -eq 1 ] ; then
	...
	wget -N https://github.com/MainFighter/NadekoBot-BashScript/raw/1.9/tmux_NadekoAutoRestart.sh &> /dev/null
	bash tmux_NadekoAutoRestart.sh
else
	if [ $choice -eq 2 ] ; then
		echo ""
		echo "Starting Nadeko with Auto Restart and Update..."
		wget -N https://github.com/MainFighter/NadekoBot-BashScript/raw/1.9/tmux_NadekoAutoRestartAndUpdate.sh &> /dev/null
