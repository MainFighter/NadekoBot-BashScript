...
if [ $choice -eq 1 ] ; then
	...
	if [ -e linuxAIO.sh ]; then
		...
		rm linuxAIO.sh
		wget -N https://github.com/Kwoth/NadekoBot-BashScript/raw/1.9/linuxAIO.sh &> /dev/null
	fi
	...
	if [ -e NadekoSystemd.sh ]; then
		...
		rm NadekoSystemd.sh
		wget -N https://github.com/MainFighter/NadekoBot-BashScript/raw/1.9/NadekoSystemd.sh &> /dev/null
	fi

	if [ -e tmux_NadekoStart.sh ]; then
		echo "Updating tmux_NadekoStart.sh..."
		rm tmux_NadekoStart.sh
		wget -N https://github.com/MainFighter/NadekoBot-BashScript/raw/1.9/tmux_NadekoStart.sh &> /dev/null
	fi
